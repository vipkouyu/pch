...
function getQuerystr(type){
	...
	switch(type){ 
		...
		case 'm100':  //判断数据库是否存在
			querystr='select * from yanbao where title = ? or url = ? ';
			break;

		case 'm122':   //移动版
			querystr='insert into yanbao set ?';
			 break;

		case 'w11':   //清洗数据
			querystr='update yanbao set ? where id= ? ';
			break;

		case 'w18':   //清洗数据
			querystr='update yanbao set washcode=1,yichang="x" where id =? ';
			break;

		case 'm110':   //清洗数据
			querystr='select * '+
			         'from cmc '+
					 'where LOWER(platform) like "%eth%" and ifnull(`status`,"")!="x"'+    //
					 'order by id desc limit ? ';
			break;
		
		case 'm120':   //清洗数据
			querystr='update cmc set watchlist=?, status="x" where id=? ';
			break;
