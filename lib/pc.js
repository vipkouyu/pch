...
exports.tripadvisor=async function(){
	//每天跑【什么值得买】前20页；
	//算法如下：
	//一、构造20页的URL；
	//二、取得所有refid；
	...
	// 一、构造20页的URL;
	var arr=[];
	for(let i=924;i<=2000;i++){

		let u='https://www.tripadvisor.com/ShowForum-g294211-i642-o'+i*20+'/-China.html';		
		arr.push(u);
	}
	...
	for(let v of arr){
		...
		var s1=v.substr(v.indexOf('-o')+2);
		var p=s1.substr(0,s1.indexOf('-')-1);
		p=p/20;
		console.log(p);
		...
		$('#SHOW_FORUMS_TABLE tr').each(function(){
			// console.log($(this).find('.feed-ver-title').text());   //取描述
			// console.log($(this).find('.feed-ver-title').children('a').attr('href'));

			let title=$(this).find('b').children().text().replace(/[\r\n]/g,"");
			title=title.toLowerCase();

			if(title.indexOf('driver')>0 || title.indexOf('guide')>0){
				let city=$(this).find('.forumcol').text().replace(/[\r\n]/g,"");			
				let link=$(this).find('b').children().attr('href');
				var re={};
				re.city=city;
				re.title=title;
				re.link=link;
				db.query('n001',re).then(function(data){
